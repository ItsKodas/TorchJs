...
export default async (guild: string, embeds: BaseMessageOptions['embeds']) => {
    ...
    channel.send({ content: Mentions, embeds: embeds }).catch(err => console.error(`Failed to Send Alert to ${channel.name} in ${channel.guild.name}:\n${err}`))
