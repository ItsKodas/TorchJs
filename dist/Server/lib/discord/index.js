                ...
                _client.on('interactionCreate', interaction => {
                    try {
                        ...
                        if (interaction.isChatInputCommand())
                            ...
                            console.log(`Interaction "${interaction.id}" does not exist on the Server!`);
                    }
                });
                //? Guilds Updates
...
//? Common Exports
export function Guild(guildId) { return new Promise((resolve, reject) => Client().then(async (client) => resolve(client.guilds.cache.get(guildId) || await client.guilds.fetch(guildId))).catch(reject)); }
export function Channel(guildId, channelId) { return new Promise((resolve, reject) => Guild(guildId).then(async (guild) => resolve((guild.channels.cache.get(channelId) || await guild.channels.fetch(channelId).catch(reject))))); }
